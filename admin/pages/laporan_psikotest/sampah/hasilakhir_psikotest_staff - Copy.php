...
CASE WHEN ROUND(SUM(nilai)/3) < 3 THEN '<img styl src=\"../../images/turun.png\" style=\"width: 20px; height: 20px\"/>' ELSE
CASE WHEN ROUND(SUM(nilai)/3) IN (3,4) THEN '<img styl src=\"../../images/tengah.png\" style=\"width: 20px; height: 20px\"/>' ELSE
'<img styl src=\"../../images/naik.png\" style=\"width: 20px; height: 20px\"/>' END END arah
FROM (
SELECT a.no_calon_kary,'G' huruf,
SUM(CASE WHEN b.nilai = 'G' THEN 1 ELSE 0 END) nilai FROM psikotest_proses_test_c a,spek_jawaban_c b
WHERE a.soalno=b.soalno and a.jawaban=b.jawaban AND
...
(SELECT ROUND(SUM(nilai)/3),
CASE WHEN ROUND(SUM(nilai)/3) < 4 THEN '<img styl src=\"../../images/turun.png\" style=\"width: 20px; height: 20px\"/>' ELSE
CASE WHEN ROUND(SUM(nilai)/3) IN (5,6,7) THEN '<img styl src=\"../../images/tengah.png\" style=\"width: 20px; height: 20px\"/>' ELSE
'<img styl src=\"../../images/naik.png\" style=\"width: 20px; height: 20px\"/>' END END arah
FROM (
...
(SELECT ROUND(SUM(nilai)/3),
CASE WHEN ROUND(SUM(nilai)/3) < 4 THEN '<img styl src=\"../../images/turun.png\" style=\"width: 20px; height: 20px\"/>' ELSE
CASE WHEN ROUND(SUM(nilai)/3) IN (5,6) THEN '<img styl src=\"../../images/tengah.png\" style=\"width: 20px; height: 20px\"/>' ELSE
'<img styl src=\"../../images/naik.png\" style=\"width: 20px; height: 20px\"/>' END END arah
FROM (
...
CASE WHEN ROUND(SUM(nilai)/2,1) <= 3.4 THEN '<img styl src=\"../../images/turun.png\" style=\"width: 20px; height: 20px\"/>' ELSE
CASE WHEN ROUND(SUM(nilai)/2,1) BETWEEN 3.5 AND 5 THEN '<img styl src=\"../../images/tengah.png\" style=\"width: 20px; height: 20px\"/>' ELSE
'<img styl src=\"../../images/naik.png\" style=\"width: 20px; height: 20px\"/>' END END arah
FROM (
...
<table border="0" style="border-style: groove; border-width: 0px; border-collapse: collapse;" align="center" width="500px">
  <tr>
    <th>HASIL TEST</th>
    <th style="padding-left: 0"><input type="text" style="width: 100%; padding-left: 10px; font-weight:  bold" readonly name="hasil_test" value="<?php 
          ...
          elseif ($rows2[0]== 'SESUAI' && $rows2[2]== '2' && $rows2[3] <= '0') {
              echo 'DIPERTIMBANGKAN DENGAN CATATAN';
          } 
